...
_LOG="$SCRIPT_DIR/../.juvant/logs/anomaly-check.log"
if [[ -f "$_LOG" ]]; then
...
if [[ -z "${JUVANT_DB_PROVIDER:-}" ]]; then
  echo "[anomaly-check] FATAL: no DB provider configured — set .db.provider (or legacy .turso_url) in $CONFIG" >&2
  exit 1
fi
if ! juvant_db_cli_ok; then
  echo "[anomaly-check] FATAL: the '$JUVANT_DB_PROVIDER' DB CLI ($(juvant_db_required_cli)) is not on PATH — re-run helpers/install-schedules.sh so the schedule's PATH includes it." >&2
  exit 1
fi
