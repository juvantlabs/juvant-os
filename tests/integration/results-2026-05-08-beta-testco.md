...
(commits `a34a02f` ADR 0010 + symlinks; `d12fe10` trivial fixes batch).
...
> Note: the original release branch numbered the symlink-registration ADR
> as 0009. After divergence with main (which had already landed
> `0009-mail-via-ms-graph-on-demand.md`), the symlink ADR was renumbered
> to 0010 during the cherry-pick onto current main. References below
> reflect the final number.
...
- **ADR 0010** ships `.claude/agents/<role>.md` symlinks for **10**
