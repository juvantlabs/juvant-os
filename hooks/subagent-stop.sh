...
# Block if THIS subagent merged a PR this session referencing an approved,
# still-unmarked pr-spec. Enforcement (CC honoring the block) confirmed live on
# 2026-07-14. Logic lives in hooks/lib/spec-marking-gate.sh (shared with the
# Stop hook); BUG-057 warn + BUG-058 spool-drain handled there.
if [[ -f "$SCRIPT_DIR/lib/spec-marking-gate.sh" ]]; then
  ...
  _L1_REASON="$(spec_marking_gate "$PARENT_SESSION_ID" "$SUBAGENT_ROLE" || echo "")"
  if [[ -n "$_L1_REASON" ]]; then
    # Block on both documented channels: JSON decision + exit 2 w/ stderr reason.
