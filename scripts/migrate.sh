...
apply_schema_patches_local() {
  ...
  local common_patches=(
    ...
    "ALTER TABLE agent_actions_log ADD COLUMN input_summary    TEXT;"
    "ALTER TABLE agent_actions_log ADD COLUMN spec_id          INTEGER;"
    "CREATE TRIGGER IF NOT EXISTS action_log_denied_requires_summary BEFORE INSERT ON agent_actions_log WHEN NEW.status = 'denied' BEGIN SELECT RAISE(ABORT,'agent_actions_log denied rows require input_summary — set it in pre-tool-use.sh') WHERE NEW.input_summary IS NULL OR trim(NEW.input_summary) = ''; END;"
    "DROP TRIGGER IF EXISTS decisions_category_check_ins;"
    "CREATE TRIGGER decisions_category_check_ins BEFORE INSERT ON decisions WHEN NEW.category IS NOT NULL BEGIN SELECT RAISE(ABORT,'decisions.category value not in canonical list — see JUVANT_OS.md schema reference') WHERE NEW.category NOT IN ('model-override','tool-matrix-change','pr-spec','gh-issue-spec','gh-project-update-spec','gh-milestone-spec','install-spec','branch-protection-spec','release-spec','deployment-spec','secret-rotation-spec','eng-output-held','disclosure-unavailable','bootstrap-action','cascade-escalation','cascade-postmortem','skill-gap','migration-watch','upstream-sync-proposal','eng-platform-spec','arch-decision','operational-violation','kb-orphan-review','legal','product','business','system-audit','incident-response','security-remediation'); END;"
    "DROP TRIGGER IF EXISTS decisions_category_check_upd;"
    "CREATE TRIGGER decisions_category_check_upd BEFORE UPDATE ON decisions WHEN NEW.category IS NOT NULL AND NEW.category != OLD.category BEGIN SELECT RAISE(ABORT,'decisions.category value not in canonical list — see JUVANT_OS.md schema reference') WHERE NEW.category NOT IN ('model-override','tool-matrix-change','pr-spec','gh-issue-spec','gh-project-update-spec','gh-milestone-spec','install-spec','branch-protection-spec','release-spec','deployment-spec','secret-rotation-spec','eng-output-held','disclosure-unavailable','bootstrap-action','cascade-escalation','cascade-postmortem','skill-gap','migration-watch','upstream-sync-proposal','eng-platform-spec','arch-decision','operational-violation','kb-orphan-review','legal','product','business','system-audit','incident-response','security-remediation'); END;"
    "CREATE TRIGGER IF NOT EXISTS decisions_approved_requires_approver_ins BEFORE INSERT ON decisions WHEN NEW.status = 'approved' AND OLD.status != 'approved' BEGIN SELECT RAISE(ABORT,'decisions status=approved requires approved_by and approved_at') WHERE NEW.approved_by IS NULL OR NEW.approved_at IS NULL; END;"
    "CREATE TRIGGER IF NOT EXISTS decisions_approved_requires_approver_upd BEFORE UPDATE ON decisions WHEN NEW.status = 'approved' AND OLD.status != 'approved' BEGIN SELECT RAISE(ABORT,'decisions status=approved requires approved_by and approved_at') WHERE NEW.approved_by IS NULL OR NEW.approved_at IS NULL; END;"
    "CREATE TRIGGER IF NOT EXISTS decisions_executed_requires_executor_ins BEFORE INSERT ON decisions WHEN NEW.status = 'executed' AND OLD.status != 'executed' AND NEW.category IN ('pr-spec','gh-issue-spec','deployment-spec','install-spec','eng-platform-spec','branch-protection-spec','release-spec','secret-rotation-spec') BEGIN SELECT RAISE(ABORT,'decisions status=executed requires executed_by, executed_at, and source_ref for spec categories') WHERE NEW.executed_by IS NULL OR NEW.executed_at IS NULL OR NEW.source_ref IS NULL; END;"
    "CREATE TRIGGER IF NOT EXISTS decisions_executed_requires_executor_upd BEFORE UPDATE ON decisions WHEN NEW.status = 'executed' AND OLD.status != 'executed' AND NEW.category IN ('pr-spec','gh-issue-spec','deployment-spec','install-spec','eng-platform-spec','branch-protection-spec','release-spec','secret-rotation-spec') BEGIN SELECT RAISE(ABORT,'decisions status=executed requires executed_by, executed_at, and source_ref for spec categories') WHERE NEW.executed_by IS NULL OR NEW.executed_at IS NULL OR NEW.source_ref IS NULL; END;"
  )
